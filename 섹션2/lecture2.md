...
* Reinforcement Learning: 알고리즘이 누적된 보상을 극대화하기 위해 조치를 취하는 것   
  * 어떤 Action에 대해 보상과 패널티를 통해 시행착오를 배우고, 시간이 지남에 따라 시행착오를 통해 Action을 개선할 수 있는 방법을 배우게 됨   
  * 인간이 배우는 과정과 가장 비슷   
   
   
# 좋은 데이터 vs 나쁜 데이터
- Unlabeled Dataset: 라벨이 없음. 즉, 이미지만 있고 개나 고양이에 대한 이미지라는 관련 태그가 아무것도 없음   
  - Unsupervised Learning 을 수행하기 위해 Unlabeled Dataset을 사용   
   
- Labeled Dataset: 이미지와 입력정보도 있고, 관련된 라벨도 있음. 즉, 고양이라는 라벨과 함께 고양이 이미지가 있고, 개라는 라벨과 함께 개 이미지도 있음   
  - Supervised Learning 을 수행하기 위해 Labeled Dataset을 사용   
   
## 좋은 데이터 VS 나쁜 데이터   
1. 표본의 양   
- 좋은 데이터: 많은 표본을 갖고 있는 것   
- 나쁜 데이터: 적은 표본을 갖고 있는 것   
모든 머신 러닝 알고리즘은 경험을 통해 학습하기 때문에 많은 표본들이 필요하고, 많은 표본은 편향되지 않는 더 좋은 데이터를 만들어냄   
하지만, 나쁜 데이터는 편향된 데이터를 만들어냄   
   
2. 누락된 포인트   
- 좋은 데이터: 누락된 포인트가 없는 것   
- 나쁜 데이터: 누락된 포인트가 있는 것   
일반적으로 머신 러닝 모델을 훈련시키기 위해선 어떤 누락된 포인트가 있으면 안됨   
누락된 포인트가 있으면 변수 가공을 수행해 데이터를 간단히 만들고 누락된 정보를 채우는 작업을 해야 함   
      
3. Feature 보유 여부
- 좋은 데이터: 관련성이 높은 중요한 특징들을 포함   
- 나쁜 데이터: 관련성이 없거나 쓸모없는 특징들을 포함   
   
4. 중복된 표본
- 좋은 데이터: 중복된 표본을 가지고 있지 않음   
- 나쁜 데이터: 중복된 표본을 가지고 있음   
따라서 변수 가공 기술 중 하나는 모든 데이터를 검토하며 중복된 데이터를 찾아서 제거하는 것   
   
### 데이터 어디서 얻을 수 있을까?
- 케글
- Center for Machine Learning and Intelligence Systems
- AWS DataSet
- ImageNet
